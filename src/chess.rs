...
pub fn state_from_fen(fen: String, state:&mut State) -> Result<(),()> {
    ...
    for c in parts[0].chars() {
        match c {
            '1'..='9' => {
                ...
                for i in 0..emp {
                    state.board[(7-rank)*8 + file + i] = None;
                }
                file += emp;
            }
            '/' => {
            ...
            'p' | 'P' | 'n' | 'N' | 'b' | 'B' | 'r' | 'R' | 'q' | 'Q' | 'k' | 'K' => {
                state.board[(7-rank)*8 + file] = Some(piece_from_char[&c]);
                file += 1;
            }
            _ => {return Err(())}
        }
    ...
    state.turn = match parts[1] {
        "w" => PieceColor::White,
        "b" => PieceColor::Black,
        _ => return Err(()),
    };

    state.castling = [false; 4]; // KQkq
    if parts[2] != "-" {
        for c in parts[2].chars() {
            match c {
                'K' => state.castling[0] = true, // White kingside
                'Q' => state.castling[1] = true, // White queenside
                'k' => state.castling[2] = true, // Black kingside
                'q' => state.castling[3] = true, // Black queenside
                _ => return Err(()),
            }
        }
    }

    state.en_passant = (8,8);
    if parts[3] != "-" {
        if parts[3].len() != 2 {
            return Err(());
        }
...
pub fn make_move_on(from: usize, to: usize, state: &mut State) {
    ...
    if piece.tp == PieceType::King {
        if from == 60 && to == 62 { // White kingside castling
            if state.castling[0] {
                state.board[61] = Some(Piece {color: PieceColor::White, tp: PieceType::Rook});
                state.board[63] = None;
                state.castling[0] = false; // Disable kingside castling
            }
        } else if from == 60 && to == 58 { // White queenside castling
            if state.castling[1] {
                state.board[59] = Some(Piece {color: PieceColor::White, tp: PieceType::Rook});
                state.board[56] = None;
                state.castling[1] = false; // Disable queenside castling
            }
        } else if from == 4 && to == 6 { // Black kingside castling
            if state.castling[2] {
                state.board[5] = Some(Piece {color: PieceColor::Black, tp: PieceType::Rook});
                state.board[7] = None;
                state.castling[2] = false; // Disable kingside castling
            }
        } else if from == 4 && to == 2 { // Black queenside castling
            if state.castling[3] {
                state.board[3] = Some(Piece {color: PieceColor::Black, tp: PieceType::Rook});
                state.board[0] = None;
                state.castling[3] = false; 
            }
        } else if piece.color == PieceColor::White {
            state.castling[0] = false; // Disable white kingside castling
            state.castling[1] = false; // Disable white queenside castling
        } else {
            state.castling[2] = false; // Disable black kingside castling
            state.castling[3] = false; // Disable black queenside castling
        }
    ...
    state.turn = if state.turn == PieceColor::White { PieceColor::Black } else { PieceColor::White };
}
...
pub fn get_valid_moves_for_raw(from: usize, state: &State) -> Vec<usize> {
    ...
    match piece.tp {
        ...
        PieceType::King => {
            ...
            // Castling
            if piece.color == PieceColor::White {
                if state.castling[0] && state.board[61].is_none() && state.board[62].is_none() {
                    valid_moves.push(62); // White kingside castling
                }
                if state.castling[1] && state.board[59].is_none() && state.board[58].is_none() && state.board[57].is_none() {
                    valid_moves.push(58); // White queenside castling
                }
            } else {
                if state.castling[2] && state.board[5].is_none() && state.board[6].is_none() {
                    valid_moves.push(6); // Black kingside castling
                }
                if state.castling[3] && state.board[3].is_none() && state.board[2].is_none() && state.board[1].is_none() {
...
pub fn get_valid_moves_for(from: usize, state: &State) -> Vec<usize> {
    ...
    for &to in &potential_moves {
        ...
        temp_state.turn = if temp_state.turn == PieceColor::White { PieceColor::Black } else { PieceColor::White };
